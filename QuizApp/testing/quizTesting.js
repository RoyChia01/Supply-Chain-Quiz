...
import { StyleSheet, Text, TouchableOpacity, View, Dimensions, Image, ActivityIndicator } from 'react-native';
          ...
          <View style={styles.optionsContainer}>
            {questions[currentQuestion]?.options.map((option, index) => (
              ...
              >
                ...
                {selectedAnswer === option && (
                  <Icon
                    name={option === questions[currentQuestion]?.answer ? 'check-circle' : 'times-circle'}
                    size={30}
                    color={option === questions[currentQuestion]?.answer ? '#90EE90' : '#FF6F6F'}
                    style={styles.icon}
                  />
                )}
              </TouchableOpacity>
...
const ProgressBar = ({ currentQuestion, totalQuestions }) => {
  ...
  const dashes = Array.from({ length: numOfDashes }, (_, i) => {
    ...
    return (
      <View key={i} style={[styles.dash, { left: `${(i / numOfDashes) * 100}%`, opacity }]} />
    );
  });
...
const styles = StyleSheet.create({
  ...
  dash: { position: 'absolute', top: '50%', width: 10, height: 6, backgroundColor: 'white', borderRadius: 2 },
